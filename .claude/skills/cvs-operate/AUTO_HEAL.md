...
Test Failed
  ...
  │     → Re-run the test
  │
  └── Any: PermissionError on /tmp HTML report files
        → AUTO-FIX: sudo rm -rf /tmp/*_html /tmp/preflight.html
        → Re-run the test
        → Root cause: previous user left files in /tmp owned by their UID
```
...
|----------|--------|---------|
| **Safe** (auto-fix) | Fix it, re-run, report | NUMA balancing, docker pull, chmod SSH key, /tmp stale file cleanup |
