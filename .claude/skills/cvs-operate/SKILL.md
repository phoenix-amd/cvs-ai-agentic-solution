...
When a new user doesn't know where to start, they paste one prompt and the
agent handles everything: mode selection, profile creation, CVS installation,
SSH setup, sanity check, and first health check.

### Step 1: Agent Asks Which Mode (automatic)

When the agent receives any CVS task, it **automatically asks** the user to
pick an operating mode using `AskUserQuestion`:
...
The user selects one, and the agent adjusts its behavior for the rest of
the session. No terminal commands needed — everything happens inside Claude.

### Step 2: Paste Your Prompt

**Magic Prompt Template (paste directly into Claude):**
```
Set up CVS on head node <IP> with worker nodes <IP1,IP2,...>.
SSH user is <username>, key is <key_path>.
Jira project is <PROJECT_KEY>.
Check everything is installed and ready, then run a quick health check.
```

### Ready-to-Use Sample Prompts (paste any of these into Claude)

| Task | Prompt |
|------|--------|
| **First-time setup** | `Set up CVS on head node 10.194.129.213 with worker 10.194.129.211. SSH user is rghaffar, key is ~/.ssh/id_ed25519. Jira project is DCCS. Check everything is installed and ready, then run a quick health check.` |
| **Install + RCCL test** | `Install CVS on head node 10.194.129.213 with worker 10.194.129.211. SSH user is root, key is ~/.ssh/id_rsa. Run health check and rccl all_reduce afterward.` |
...
| **Network validation** | `Check RDMA connectivity and run IB bandwidth test on 10.194.129.213 and 10.194.129.211. SSH user root, key ~/.ssh/id_rsa.` |
| **Pre-training readiness** | `Is the cluster 10.194.129.213 with worker 10.194.129.211 ready for distributed training? SSH user root, key ~/.ssh/id_rsa.` |
...
### What Happens After You Paste (10-Step Sequence)

```
0. Agent asks: "Which mode? Interactive / Autonomous / Batch"
1. Save cluster profile → ~/.cvs_agent/cluster_profile.json
2. SSH to head node → check CVS installed
   MISSING → install from source (git clone + pip install -e .)
...
### Alternative: Terminal Launchers (for CI/CD only)

For automation outside of Claude (cron jobs, CI pipelines), terminal launchers
are available. These are NOT needed for normal use inside Claude.

```bash
# From a terminal (not inside Claude):
...
**Auto-install CVS (always from source — `pip install cvs` does NOT work):**
```bash
# CVS is NOT on PyPI — always install from source
ssh <headnode> 'cd ~ && \
    git clone https://github.com/ROCm/cvs.git cvs && \
    cd cvs && \
...
# Verify
ssh <headnode> 'source ~/cvs/.cvs_venv/bin/activate && cvs --version'
```

**IMPORTANT**: Do NOT use `pip install cvs` — the CVS package is not published
on PyPI. Always clone from GitHub and install from source. The venv lives at
`~/cvs/.cvs_venv` on the head node. Always activate it before running CVS:
`source ~/cvs/.cvs_venv/bin/activate`
...
### Step 3.5: Pre-Run Cleanup (MANDATORY before every test)

**Always clean stale /tmp artifacts from previous runs before running any test.**
Previous users or previous runs may leave files owned by a different user in
`/tmp`, causing `PermissionError` when CVS tries to write reports.

```bash
# Run this on the head node BEFORE every test suite
ssh <headnode> '
  # Clean stale CVS report directories (may be owned by other users)
  sudo rm -rf /tmp/preflight_checks_html /tmp/preflight.html /tmp/preflight.log 2>/dev/null
  sudo rm -rf /tmp/rccl_*_html /tmp/host_configs_*_html 2>/dev/null
  sudo rm -rf /tmp/agfhc_*_html /tmp/transferbench_*_html 2>/dev/null

  # If sudo is not available, try without (will only remove own files)
  rm -rf /tmp/preflight_checks_html /tmp/preflight.html /tmp/preflight.log 2>/dev/null
  rm -rf /tmp/rccl_*_html /tmp/host_configs_*_html 2>/dev/null
  rm -rf /tmp/agfhc_*_html /tmp/transferbench_*_html 2>/dev/null

  echo "Pre-run cleanup complete"
'
```

**Why this matters**: CVS writes HTML sub-reports to `/tmp/<suite>_html/`.
If these directories exist from a previous run by a different user, the
current run gets `PermissionError` and the HTML report generation fails
(even though the actual tests pass). This causes confusing `INTERNALERROR`
messages that look like test failures but are just report-writing issues.

**Rule**: Always run this cleanup before `pytest`. Never skip it.
...
6. **Always print results in terminal** — present a markdown table with per-test
   PASS/FAIL status directly in the conversation. Never rely solely on HTML reports.
7. Present clear summary with next steps
8. **Serve HTML report** (see Report Delivery below)

**Terminal output is mandatory.** After every test run, the agent MUST print a
results summary table like this directly in the conversation:

```
| Test | Status | Details |
|------|--------|---------|
| test_node_reachability | PASS | 2/2 nodes reachable |
| test_rocm_version | PASS | 7.2.0 on all nodes |
| test_rdma_connectivity | FAIL | mlx5_5 port down on node 1 |
```

For RCCL tests, also print bandwidth results:

```
| Size | AlgBW (GB/s) | BusBW (GB/s) | Errors |
|------|-------------|-------------|--------|
| 8 GB | 58.0 | 108.7 | 0 |
| 16 GB | 58.0 | 108.7 | 0 |
```

The user should never need to open an HTML report to see basic pass/fail results.
...
### 0. Verify MPI default-key SSH (CRITICAL — do before anything else)

OpenMPI spawns worker daemons (`orted`) via SSH **without a `-i` flag**. It
falls back to `~/.ssh/id_rsa` on the head node. If that key is not authorized
on workers, RCCL appears to succeed but all ranks land on the head node
(single-node behavior, `nodes: 1` in results).

```bash
# Verify head's id_rsa reaches each worker without -i
ssh <headnode> 'ssh -o BatchMode=yes -o ConnectTimeout=5 <worker_ip> hostname'

# Also verify bare mpirun distributes ranks correctly
ssh <headnode> 'mpirun --np 2 --host <head_ip>,<worker_ip> hostname'
# Must print two different hostnames — one per node
```

If either check fails: run cvs-preflight-setup Check 2d to push `id_rsa.pub`
to the worker, then verify again before continuing.
...
| `ORTE was unable to reliably start one or more daemons` | MPI orted uses `~/.ssh/id_rsa` (no `-i` flag) — that key not authorized on workers | Push head's `id_rsa.pub` to each worker's `authorized_keys`; verify with `ssh -o BatchMode=yes <worker> hostname` |
| `PermissionError: /tmp/preflight_checks_html/...` | Previous user left `/tmp` files owned by their UID | `sudo rm -rf /tmp/preflight_checks_html /tmp/preflight.html`; see Step 3.5 |
| `PermissionError: /tmp/preflight.html` | Same as above — stale report file from previous run | Clean `/tmp` stale files (Step 3.5), then re-run |
| `pip install cvs` fails "No matching distribution" | CVS is NOT on PyPI | Always install from source: `git clone` + `pip install -e .` |
| `file or directory not found: cvs/tests/rccl/rccl_multinode_cvs.py` | Wrong test script path (renamed in CVS 1.0) | Use `cvs/tests/rccl/rccl_perf.py` for perf tests, `rccl_regression.py` for regression |
...
16. Don't trust RCCL `nodes` field without checking — `nodes: 1` means all ranks landed on head (MPI key issue); verify with `mpirun --np 2 --host <head>,<worker> hostname` before concluding the test is multi-node
17. Don't use `pip install cvs` — CVS is NOT on PyPI; always install from source with `git clone` + `pip install -e .`
