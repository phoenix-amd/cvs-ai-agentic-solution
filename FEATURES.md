...
**18 features | 34 test suites | Pure agent layer | Zero fork maintenance**
...
16. [Zero-Prompt Auto Mode](#16-zero-prompt-auto-mode)
17. [Pre-Run /tmp Cleanup](#17-pre-run-tmp-cleanup)
18. [Terminal Results Output](#18-terminal-results-output)
...
### How It Works (Everything Inside Claude)

1. Open Claude Code (`claude` from terminal)
2. Paste your prompt (see samples below)
3. Agent asks: **"Which operating mode?"** → Interactive / Autonomous / Batch
4. You pick one → agent adjusts behavior for the rest of the session

```
User:  Install CVS on head node 10.194.129.213 with worker 10.194.129.211.
       SSH user is root, key is ~/.ssh/id_rsa.
       Run health check and rccl all_reduce afterward.

Agent: Which operating mode would you like?
...
Agent: Mode: Autonomous. Running full workflow...
       → SSH to head node... connected
       → CVS not found, installing... done (v1.4.0)
       → Generating cluster.json... done
       → Running preflight_checks... all passed
       → Running rccl_perf -k all_reduce... 348 GB/s (target: 330) PASS
```
...
|----------|:-----------:|:----------:|:-----:|
| Agent asks "which mode?" | Yes | Yes | Yes |
| Agent confirms before SSH, CVS, pytest | **Yes (agent asks)** | No — logs and proceeds | No — silent |
| Explains each step | Detailed | Brief log line | Minimal |
| Best for | First time, debugging | Routine validation, overnight | Repeat runs |

### Common Sample Prompts

| Task | Prompt to Paste |
|------|----------------|
| First-time setup + health check | `Set up CVS on head node 10.194.129.213 with worker 10.194.129.211. SSH user is root, key is ~/.ssh/id_rsa. Check everything is installed and ready, then run a quick health check.` |
| RCCL performance test | `Run RCCL all_reduce and all_gather on 10.194.129.213 and 10.194.129.211. SSH user root, key ~/.ssh/id_rsa.` |
| Full overnight qualification | `Run full cluster qualification overnight on 10.194.129.213 with worker 10.194.129.211. SSH user root, key ~/.ssh/id_rsa. Auto-heal what you can, escalate hardware failures to Jira project DCCS.` |
| GPU burn-in | `Run GPU burn-in on head node 10.194.129.213. SSH user root, key ~/.ssh/id_rsa.` |
...
| Single-node quick check | `Run preflight and platform check on 10.194.129.213 (single node). SSH user root, key ~/.ssh/id_rsa.` |
...
## 16. Zero-Prompt Auto Mode

### Why This Exists

In autonomous mode, Claude Code's built-in security heuristics still flagged
~25+ commands for manual approval during a real validation run. Patterns like
heredocs, SSH with `-o` flags, inline python3, and `/tmp` reads triggered
"potential bypass" or "obfuscation" warnings — even though they are standard
CVS operations.

### Value

- **True zero-prompt execution**: All CVS operations run without stopping for approval
- **No workflow interruption**: User selects "Autonomous" and walks away
- **Security preserved**: Deny hooks still block `rm -rf /`, `reboot`, force push

### Who Decides: Configuration (Pre-Configured in settings.local.json)

The `autoMode.allow` rules in `.claude/settings.local.json` tell the auto mode
classifier which patterns are safe for CVS operations. The deny hooks in
`.claude/settings.json` remain active as a safety net.
...
## 17. Pre-Run /tmp Cleanup

### Why This Exists

CVS writes HTML sub-reports to `/tmp/<suite>_html/`. On shared clusters,
previous users leave these directories owned by their UID. When a new user
runs the same test, CVS gets `PermissionError` trying to write to the
existing directory — even though the actual tests pass.

### Value

- **No PermissionError surprises**: Clean `/tmp` before every test
- **Saves 10-15 min debugging**: Previously required manual investigation
- **Auto-healable**: If missed, the agent detects PermissionError and cleans+retries

### Who Decides: Agent (Automatic Before Every Test)

The agent runs cleanup before every `pytest` invocation — no user action needed.

### How It Works

```bash
# Runs on head node before every test suite
sudo rm -rf /tmp/preflight_checks_html /tmp/preflight.html /tmp/preflight.log
sudo rm -rf /tmp/rccl_*_html /tmp/host_configs_*_html /tmp/agfhc_*_html
```

---

## 18. Terminal Results Output

### Why This Exists

Users shouldn't need to open an HTML report to see basic pass/fail results.
When running via SSH or WSL, HTML report viewing requires HTTP server setup.
The results should be immediately visible in the conversation.

### Value

- **Instant visibility**: See results without opening a browser
- **Copy-pasteable**: Markdown tables easy to share in Slack/email
- **No dependency**: Works even when HTTP serving fails

### Who Decides: Agent (Automatic After Every Test)

The agent prints a markdown table after every test suite completes.
